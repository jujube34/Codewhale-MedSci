# Codewhale-MedSci 与原生 TUI 会话互通计划

状态：待实施。2026-09-19 基于当前工作区源码检查编写；本文不表示互通已经实现，也不执行迁移。

## 1. 产品目标与完成定义

Codewhale-MedSci 是 Codewhale 的简易 GUI。用户切换 GUI 与 Terminal 时，切换的是交互界面，不应重新创建一套对话历史。

目标场景：

1. GUI 在目录 A 执行并保存会话 S，关闭或释放 S。
2. 同一系统用户在自己的 Terminal 中进入 A，以正常原生 Codewhale 配置启动 TUI，在原生会话列表中找到 S，并通过原生续接入口继续。
3. TUI 保存、释放 S 后，GUI 的会话列表仍显示同一个 S，继续时能读到 TUI 新增内容。
4. 上述往返至少完成两轮，用户可见的 session ID 不变，不产生“每次切换一个副本”。
5. 相同目录可以并行打开任意多个不同会话。相同会话只允许一个执行所有者；其他窗口可以浏览已持久化历史。

不改变原生 TUI 的默认新建／自动恢复偏好。互通保证原生列表和显式续接可用，不把“进入相同目录”强行解释为接管某个会话。

## 2. 设计原则

- `SavedSession` / `SessionMetadata.id` 是两端共同的用户会话身份，`SessionManager` 是其持久化入口。
...
| 保存会话与 Runtime 前缀校验 | `ThreadRecord.session_id`、`SavedSessionCheckpoint`、`set_thread_session_checkpoint`、`saved_session_prefix` | 需要处理交替接续后的前缀更新，不能跳过摘要校验 |
| 原生引擎可以接收宿主 session ID | `crates/tui/src/core/engine.rs`：`EngineConfig.session_id` | 不需要 GUI 发明另一个对话身份体系 |
| 当前 GUI 覆盖原生 home 并指定桌面 Runtime 路径 | `apps/desktop/src-tauri/src/agent.rs`、`main.rs` | 这使普通 Terminal 与 GUI 看不到同一原生会话库，必须调整 |
...
3. TUI 修改 SavedSession 后，旧 thread 的 `messages_sha256` 可能不再匹配。当前 `saved_session_prefix` 会拒绝这种状态；这是数据完整性保护，不能删除。
...
- Terminal 若选择了另一个显式 home，需要 GUI 使用同一个 home；不扫描全机其他用户或其他历史库。
...
5. 当 TUI 已推进同一 SavedSession 时，在取得所有权后，由原生逻辑从最新 SavedSession 重新建立执行前缀／checkpoint。旧执行已覆盖的 turns 不得重复追加，新内容不得丢失。
6. 对无法证明关系的历史分歧明确返回冲突，不静默选择一边、不覆盖、不自动 fork。用户主动 fork 才创建新会话。
7. 保存成功后才能标记可供另一个宿主接续；保存失败时保留原生恢复证据，不显示“已保存”。
8. 先停止并排空当前执行、完成保存，再释放所有权及子进程。不能通过杀死其他宿主抢占。

实施前必须核实原生 TUI 是否已经覆盖上述每个所有权阶段；缺口在原生处补齐，并使 TUI 与 GUI 调用同一逻辑。不得另加 GUI 独占锁作为跨界面安全保证。
...
宿主切换不自动升级权限。尤其不能沿用当前 GUI 在续接时无条件写入 Full Access 的行为，把原来在 TUI 中受限的会话静默升级。API 凭据仍通过各宿主的配置／凭据机制提供，不写入 SavedSession；缺少原 provider 路由时明确说明，不静默切换服务商。
...
- 一次失败不能破坏已存在的 TUI 会话。部分完成后重跑不重复导入、不覆盖较新的历史。
...
保留旧格式接入时，应放在明确的原生兼容入口，并仅用于迁移／读取旧记录；不允许新会话继续写旧桌面体系。只有原生会话锁已经证明覆盖 GUI 和 TUI 后，才能移除 GUI 过渡锁。

## 6. 实施顺序

### P0：核对原生契约并确定最小变更

沿上表读取实际调用链，确认 TUI 的创建、恢复、自动保存、执行所有权和 Engine session ID 流程。记录现有 HTTP/stdio 入口可直接使用的部分及缺口，特别是完整快照字段、stable session ID、绑定保持和双向 checkpoint。
...
先实现并编译运行，再选择性补保护数据完整性的测试。退出条件是原生层可以完成同一 SavedSession 的多次宿主式往返，不依赖 GUI 自制数据。
...
退出条件：GUI → TUI → GUI 往返可用；浏览历史不需要启动推理；关闭某窗口不影响其他不同会话。
...
在 Windows 和 macOS 分别用从同一源码构建的 GUI 与 CLI/TUI 验证完整闭环。确认发行包包含匹配 sidecar；旧版或不兼容原生 CLI 明确提示版本问题，不把失败伪装成空历史。
...
| 场景 | 必须观察到的结果 |
| --- | --- |
| GUI 创建 S → 原生 TUI 列表／显式续接 | 同一 SavedSession ID；无需复制文件或给 Terminal 注入 GUI 私有 home |
| TUI 创建 S → GUI 列表／续接 | 同一 session；工作目录、路由和原生可恢复信息正确 |
| GUI → TUI → GUI → TUI | 消息增量不丢失、不重复，列表不产生新副本，无 checkpoint 分歧被忽略 |
| 同目录两个不同 session | 可同时运行，不引入工作区单实例限制 |
| 两界面同时接续同一 S | 一个执行所有者；另一方可读历史并获得明确占用反馈，不杀进程、不抢锁 |
| 旧 Runtime 所有权与路径绑定不匹配 | 原生拒绝，原始数据不变 |
| 重启／崩溃／写入失败 | 有明确原生恢复路径，不能静默覆盖新历史；另一宿主不会读到半份快照 |
| 改名／归档后另一界面保存 | 生命周期字段不回滚 |
| 含工具、思考、附件和压缩历史 | 结构与配对保留，费用未知标记不变；缺失材料明确报错 |
| TUI 受限会话从 GUI 续接 | 不静默升级为 Full Access |
| 旧历史迁移中断并重试 | 不重导、不覆盖，不改动仍被占用的原生 store |
| 中文、空格、目录别名和显式 home | 遵循原生路径比较与工作区过滤，无跨工作区串会话 |
| 普通用户 Terminal 的独立 CLI | 使用兼容版本时能实际发现并续接，不只验证 GUI 自带 sidecar |

优先使用原生现有 fixture、模拟 provider 或本地 transport，覆盖工具／思考／压缩和中断恢复；不为验证历史互通发起付费模型请求。新增测试只保护已经实现的高风险行为，并尽量证明去掉修复后会失败。针对修改运行编译与相关检查，不机械运行全 workspace。
